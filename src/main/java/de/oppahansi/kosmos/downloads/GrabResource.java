...
public class GrabResource {
  ...
  public Response grab(@PathParam("movieId") UUID movieId, GrabRequest request) {
    return grabService
        .grab(movieId, request)
        .map(
            grab ->
  ...
  public Response grabFile(
    ...
    return grabService
        .grabFile(movieId, downloadClientId, releaseTitle, content, filename)
        .map(
            grab ->
                Response.status(Response.Status.CREATED).entity(GrabResponse.from(grab)).build())
        .orElse(Response.status(Response.Status.NOT_FOUND).build());
